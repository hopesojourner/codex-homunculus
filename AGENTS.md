...
4. Before the final response, if the task produced a durable workflow lesson or the user corrected Codex behavior, ask before storing it, then run `node plugins/codex-homunculus/scripts/homunculus.mjs learn --domain "<domain>" --trigger "<specific trigger>" --action "<specific action>" --evidence "<brief evidence>"`.
