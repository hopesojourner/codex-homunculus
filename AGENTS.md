...
1. Before planning or editing, run `C:\Users\Gchen\.codex\bin\codex-homunculus.cmd start`.
2. Apply relevant learned instincts with `C:\Users\Gchen\.codex\bin\codex-homunculus.cmd apply --context "<short task summary>"`.
...
5. Before the final response, if the task produced a durable workflow lesson or the user corrected Codex behavior, ask before storing it, then run `C:\Users\Gchen\.codex\bin\codex-homunculus.cmd learn --domain "<domain>" --trigger "<specific trigger>" --action "<specific action>" --evidence "<brief evidence>"`.
